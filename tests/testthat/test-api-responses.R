...
test_that("JobSubmission class has expected fields", {
  ...
  expect_true(
    "compoundIds" %in% names(job) || "alignedFeatureIds" %in% names(job)
  )
  expect_true("fallbackAdducts" %in% names(job))
  expect_true("formulaIdParams" %in% names(job) || "sirius" %in% names(job))
})
...
test_that("RSirius package version is as expected", {
  ...
  expect_equal(
    ...
    "6.3.3",
    label = paste(
      "RSirius version changed from expected.",
      "Current:",
      pkg_version,
...
test_that("FeatureImport toJSON/fromJSON round-trip works", {
  ...
  expect_true(
    "toJSON" %in% names(feature) || "toJSONString" %in% names(feature)
  )
})
