...
spectra are present and no `ms_column_name` is provided, `import()` automatically
groups MSn spectra by acquisition order: within each file (`dataOrigin`), a new
group starts whenever a new MS2 `precursorMz` is encountered, and any subsequent
higher-level scans (MS3+) are assigned to the same group as their preceding MS2.
...
Note that MS3+ spectra are passed to SIRIUS under the `ms2Spectra` field, which
is the expected behavior — SIRIUS bundles all MSn (level ≥ 2) data together
within a feature.
