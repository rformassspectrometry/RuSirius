...
.groupMSnIndex <- function(spectra) {
  ...
  if (anyNA(origins)) {
    stop(
      "'dataOrigin' must not contain NA values. Ensure all ",
...
.createpeaks <- function(matrices) {
  if (nrow(matrices) == 1) {
    return(list(RSirius::SimplePeak$new(
      mz = matrices[1],
      intensity = matrices[2]
    )))
  }
  res <- apply(matrices, 1, function(x) {
    RSirius::SimplePeak$new(mz = x[1], intensity = x[2])
  })
  return(res)
}
...
.importSpectraChunk <- function(chunk, sirius, ms_column_name, data, adducts) {
  ...
  for (pos in seq_along(chunk)) {
    ...
    fdata[[pos]] <- .createfeatures(data = bd_subset, idx = i, adduct = adduct)
  }
  ...
  if (is.null(result) || length(result) == 0) {
    warning(
      "Sirius API did not return any features for this chunk. ",
...
.clean_output <- function(output) {
  paste(
    ...
    paste(
      vapply(
        output$affectedAlignedFeatureIds,
        function(x) {
          paste(x, collapse = ", ")
        },
        character(1)
...
.sdk_connect <- function(sdk, port = integer(), verbose = FALSE) {
  call_fn <- function() {
    if (length(port)) {
      sdk$attach_to_sirius(sirius_port = port)
    } else {
