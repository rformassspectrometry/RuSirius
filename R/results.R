...
setMethod(
  ...
  function(
    ...
    res <- switch(
      result.type,
      formulaId = .fetch_aligned_features(
        object,
        opt_fields = c("topAnnotations")
      ),
      structure = .fetch_aligned_features(
        object,
        opt_fields = c("topAnnotations")
      ),
      deNovo = .fetch_aligned_features(
        object,
...
.fetch_aligned_features <- function(sirius, opt_fields) {
  res <- sirius@api$features_api$GetAlignedFeatures(
    sirius@projectId,
  ...
  flattened <- lapply(res, function(x) {
    as.data.frame(.flatten_list(x$toSimpleType()), stringsAsFactors = FALSE)
  })
  ...
  process_function <-
    switch(
      ...
      formulaId = function(feature_id) {
        .process_single_feature(sirius, feature_id, top = topFormula)
      },
      structureDb = function(feature_id) {
        form_res <- .process_single_feature(
          sirius,
          feature_id,
          top = topFormula
        )
        ...
        .structure_for_one_feature(sirius, feature_id, form_res, topStructure)
      },
      compoundClass = function(feature_id) {
        ...
        .compound_for_one_feature(sirius, feature_id, form_res)
      },
      deNovo = function(feature_id) {
        ...
        .de_novo_for_one_feature(sirius, feature_id, form_res, topStructure)
      },
      spectralDbMatch = function(feature_id) {
        .spectral_match_for_one_feature(
          sirius,
          feature_id,
      ...
      fragTree = function(feature_id) {
        ...
        .fragTree_for_one_feature(sirius, feature_id, form_res)
      },
...
.process_single_feature <- function(sirius, feature_id, top = integer()) {
  ...
  if (length(candidates) > top) {
    candidates <- candidates[seq_len(top)]
  }
...
.spectral_match_for_one_feature <- function(sirius, feature_id, top) {
  candidates <- sirius@api$features_api$GetSpectralLibraryMatches(
    sirius@projectId,
    feature_id
  )
  if (length(candidates) > top) {
...
.process_compound_classes <- function(parsed_json) {
  # Process NPC data
  npc_df <- dplyr::bind_rows(lapply(
    names(parsed_json)[seq_len(3)],
    function(section) {
      cbind(
        section = section,
        as.data.frame(parsed_json[[section]], stringsAsFactors = FALSE)
      )
    }
  ))
  # Process ClassyFire lineage and alternatives
  cf_lineage_df <- .convert_to_dataframe(
    parsed_json$classyFireLineage,
    "classyFireLineage"
  )
  cf_alternatives_df <- .convert_to_dataframe(
    parsed_json$classyFireAlternatives,
...
.map_sirius_to_xcms <- function(sirius, sirius_id) {
  ...
  if (length(matching_xcms) == 0) {
    warning("No matching XCMS ID found for the given Sirius ID: ", sirius_id)
